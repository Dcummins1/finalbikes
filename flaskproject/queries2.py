...
#     sql = "SELECT TOP 3 last_updated FROM dublin_bikes_dynamic;"
    sql = "SELECT * From dublin_bikes_dynamic WHERE number = {0} ORDER BY last_update DESC Limit 1;".format(x)
#     sql = "SELECT top 288 available_bikes, available_bike_stands From dublin_bikes_dynamic WHERE number = {0} ORDER BY last_update DESC Limit 1;".format(x)
    curs.execute(sql)
    for row in curs.fetchall():
#         print (datetime.datetime.fromtimestamp(row[6]/1000))
        print(row)
